...
import tkinter as tk  # Ajout de l'importation pour tkinter constants
    ...
    try:
        ...
        try:
            ...
            if usb_devices:
                ...
                wav_filename = os.path.join(usb_mount_point, f'acquisition_{usb_name}_{time.strftime("%Y%m%d-%H%M%S")}.wav')
                create_wav(bufferCompleteA, wav_filename)
            else:
                print("Aucun périphérique de stockage USB monté trouvé. Impossible de sauvegarder le fichier WAV.")
...
if __name__ == "__main__":
    ...
    frame.pack(expand=tk.YES, fill=tk.BOTH)  # Correction ici
